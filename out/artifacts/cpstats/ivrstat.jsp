...
%>
	<body>
		<form name="form1" method="post" action="ivrstat.jsp?action=search&type=<%=types%>" id="form1" onsubmit="selectform();">
			<div>
				&nbsp;&nbsp;移动IVR数据统计-->结算时间范围:
				...
				<hr />
					...
					<table align=center class=table1>
						<tr><td>时间</td><td>CP公司</td><td>拨打端口</td><td>资费</td><td>拨打时长（分钟）</td><td>总信息费</td></tr>
						<%
                        int counts=0;
						Double totalcounts=0.0;
						int numa=0;
						int total=0;
						String[][] cs =stat.getivrstat(userName,starttime,endtime);
						...
						<tr>
							...
							<td><%=stats[3]%></td>
							<%
								if("125905466".equals(stats[2].subSequence(0, 9))||"125904825".equals(stats[2].subSequence(0, 9))){
							%>
								<td>(拨打次数)<%=stats[6]%></td>
							<%	
								}else{
