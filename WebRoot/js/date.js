/**//**
	 ...
	 * tiannet添加了时间选择功能、select,object标签隐藏功能，还有其它小功能。 使用方法： (1)只选择日期 <input
	 * type="text" name="date" readOnly onClick="setDay(this);"> (2)选择日期和小时
	 * <input type="text" name="dateh" readOnly onClick="setDayH(this);">
	 * (3)选择日期和小时及分钟 <input type="text" name="datehm" readOnly
	 * onClick="setDayHM(this);"> 设置参数的方法 (1)设置日期分隔符
	 * setDateSplit(strSplit);默认为"-" (2)设置日期与时间之间的分隔符
	 * setDateTimeSplit(strSplit);默认为" " (3)设置时间分隔符
	 * setTimeSplit(strSplit);默认为":" (4)设置(1),(2),(3)中的分隔符
	 * setSplit(strDateSplit,strDateTimeSplit,strTimeSplit); (5)设置开始和结束年份
	 * setYearPeriod(intDateBeg,intDateEnd) 说明： 默认返回的日期时间格式如同：2005-02-02 08:08
	 */  
// ------------------ 样式定义 ---------------------------//
// 功能按钮同样样式
...
function setMonth(obj){   
 ...
 var strValue = tiannetTrim(tiannetOutObject.value);   
 if( strValue != "" ){   
  tiannetInitDate(strValue.substring(0,7));   
 }   
 tiannetPopCalendar();   
}     
// 用户主调函数－选择日期和小时
...
document.write('</select>');   
// document.write('</div>');
// document.write('<div align="center" id="divTiannetTimeText"
// Author="tiannet">');
...
function tiannetHideObject(strTagName) {   
 ...
 {   
  ...
  // alert("控件左端:" + x + "select左端" + (objLeft + objWidth) + "控件底部:" + (y+h) +
	// "select高:" + objTop);
